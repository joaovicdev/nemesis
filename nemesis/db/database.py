...
class Database:
    ...
    async def create_project(self, project: Project) -> Project:
        async with self._cursor() as cur:
            await cur.execute(
                """
                INSERT INTO projects (id, name, targets, context, status, mode, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    project.id,
    ...
    async def update_project(self, project: Project) -> None:
        ...
        async with self._cursor() as cur:
            await cur.execute(
                """
                UPDATE projects
                SET name=?, targets=?, context=?, status=?, mode=?, updated_at=?
                WHERE id=?
                """,
                (
                    project.name,
    ...
    async def create_finding(self, finding: Finding) -> Finding:
        async with self._cursor() as cur:
            await cur.execute(
                ...
                """,
                (
                    finding.id, finding.project_id, finding.session_id,
                    finding.title, finding.description, finding.severity.value,
                    finding.status.value, finding.confidence,
                    finding.target, finding.port, finding.service,
                    json.dumps(finding.cve_ids), finding.tool_source,
                    finding.raw_evidence, finding.remediation,
                    json.dumps(finding.related_finding_ids),
                    finding.discovered_at.isoformat(), finding.updated_at.isoformat(),
                ),
            )
        await self._conn.commit()  # type: ignore[union-attr]
        return finding
    ...
    async def append_chat(self, entry: ChatEntry) -> None:
        async with self._cursor() as cur:
            await cur.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?)",
                (
                    entry.id, entry.project_id, entry.session_id,
                    entry.role, entry.content, entry.timestamp.isoformat(),
                ),
            )
        await self._conn.commit()  # type: ignore[union-attr]
    ...
    async def create_task(self, task: TaskRecord) -> TaskRecord:
        async with self._cursor() as cur:
            await cur.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    task.id, task.project_id, task.session_id,
                    task.label, task.tool, task.status,
                    task.note, task.created_at.isoformat(),
                ),
            )
        await self._conn.commit()  # type: ignore[union-attr]
        return task
